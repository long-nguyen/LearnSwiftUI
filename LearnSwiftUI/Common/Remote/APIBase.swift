...
#if DEBUG
let API_BASE_URL = "http://weatherapp-stg.activeuser.co/v1/api/"
#else
let API_BASE_URL = ""
#endif
struct APIPath {
...
class APIBase {
    ...
    func executeRequest(_ method: HTTPMethod, _ path: String?, _ params: [String: Any]? = nil, _ headers: [String: String]? = nil, _ completion: APICompletionHandler? = nil) {
        ...
        //request
        let request = AF.request(self.buildUrl(path), method: method, parameters: params, encoding: encoding, headers: preparedHeaders)
            .validate()
...
//                    self?.processNetworkError(response, completion: completion)
    ...
    private func mergeHeaders(_ headers: [String:String]?) -> HTTPHeaders {
